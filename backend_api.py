...
class JobStatus(BaseModel):
    job_id: str
    status: str  # pending, downloading, transcribing, analyzing, generating, completed, failed
    progress: int  # 0-100
    message: str
    clips: List[dict] = []
...
# ═══════════════════════════════════════════════════════════════════════════════
# IN-MEMORY JOB STORE (for single-server deploy; use Redis for multi-server)
# ═══════════════════════════════════════════════════════════════════════════════
...
def get_video_info(video_path: Path) -> dict:
    ...
    try:
        ...
        return {
            "duration": duration,
            "width": int(streams.get("width", 1920)),
            "height": int(streams.get("height", 1080))
        }
    except:
        return {"duration": 0, "width": 1920, "height": 1080}
...
def generate_mock_transcription(audio_path: Path) -> list:
    """Generate realistic mock transcription when no API key"""
    ...
    viral_phrases = [
        ...
        "This is the moment that went viral",
        "Wait for it...",
...
def find_viral_moments(transcript: list, video_duration: float, clip_count: int, clip_duration: int) -> list:
    """Find viral-worthy moments from transcript"""
    ...
    for seg in scored_segments:
        ...
        if not overlap:
            ...
            selected.append({
                ...
                "end": round(end, 2),
                "text": seg["text"],
                "viral_score": min(99, max(70, seg["score"] + 70))
            })
            used_ranges.append((start, end))

    # If we don't have enough clips, fill with evenly spaced segments
    if len(selected) < clip_count:
...
def create_short_clip(
    ...
    """Create a vertical short clip with burned-in captions"""
    ...
    if caption and template:
        # Split caption into lines (max 3 words per line for mobile)
        words = caption.split()
        lines = []
        current_line = []
        for word in words:
...
async def process_video_job(job_id: str, source: str, is_url: bool, settings: dict):
    ...
    try:
        ...
        viral_clips = await asyncio.to_thread(
            find_viral_moments,
            transcript,
            duration,
            clip_count,
...
async def download_clip(clip_id: str):
    ...
    return FileResponse(
        ...
        filename=f"rahman-shorts-{clip_id}.mp4"
    )
...
async def edit_clip(clip_id: str, edit: ClipEdit):
    """Edit a clip's caption and regenerate"""
    # Find the original job and video
    original_clip = None
    ...
    # Get template
    templates = load_templates()
    template = next((t for t in templates.get("templates", []) if t["id"] == edit.template_id), None)
    if not template:
        template = templates.get("templates", [{}])[0]

    # Find original video (we'd need to store this, simplified here)
...
# Cleanup old clips periodically (keep last 50 clips)
